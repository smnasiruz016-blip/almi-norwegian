...
export async function generateMetadata({ params }: { params: Params }): Promise<Metadata> {
  ...
  return {
    title: { absolute: `Study ${s.name} in Denmark — Danish-language pathway | AlmiDanish` },
    description: `The Danish-language route for studying ${s.name} in Denmark — typical CEFR level and honest readiness practice, by country of origin.`,
    alternates: { canonical: `/study-in-denmark/${s.slug}` },
  };
}
    ...
    <main className="bg-almi-bg text-almi-text">
      <div className="mx-auto max-w-4xl px-6 py-12">
        <nav aria-label="Breadcrumb" className="mb-6 text-xs text-almi-text-muted">
          <Link href="/study-in-denmark" className="hover:text-almi-coral">Study in Denmark</Link> / {s.name}
        </nav>
        <h1 className="text-3xl font-semibold text-almi-ink sm:text-4xl">Study {s.name} in Denmark</h1>
        <p className="mt-3 max-w-2xl text-base text-almi-text">
          The University of Copenhagen, Aarhus University and the University of Southern Denmark offer {s.name} programmes; most
          Danish-taught courses ask for around B2. Pick your country of origin for the language pathway, or start
          practising the Danish exams now.
        </p>
        <div className="mt-6">
          <Link href="/exams" className="text-sm font-semibold text-almi-coral hover:underline">See the Danish exam guides →</Link>
        </div>
