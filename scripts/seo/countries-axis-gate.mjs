...
//
//  1. SELF-ORIGIN. src/data/seo/countries.json listed Norway itself, so the
//     matrix advertised "Study in Norway from Norway". almi-dutch and
//     almi-icelandic had the identical bug. axes.ts even carried the comment
//     "origins only (Norway excluded)" — the label was localized, the fact was
...
const SELF_ISO = "NO"; // this product's own country — never a valid origin of itself
