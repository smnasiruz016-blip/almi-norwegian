...
import { getItems } from "../../src/lib/no/items";
import { ALL_EXAMS } from "../../src/lib/no/registry";
import type { NorwegianSkill } from "../../src/lib/no/types";
...
for (const exam of ALL_EXAMS) {
  const counts = exam.skills.map((skill) => {
    const n = getItems({ exam: exam.exam, skill: skill as NorwegianSkill }).length;
    modules++;
    total += n;
